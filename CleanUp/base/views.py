...
def task_approve(request, pk):
    ...
    task = Tasks.objects.get(id=pk)
    print(task.assigned)
    userd, created = UserData.objects.get_or_create(user=task.assigned)
    if (request.user.is_staff):
...
def task_form(request):
    ...
    if (request.user.is_staff):
        ...
        if request.method == "POST":
            ...
            category_n, created = Category.objects.get_or_create(
                name=category_name)
            location_name = request.POST.get('location')
            location_n, created = Location.objects.get_or_create(
                name=location_name)
        ...
        context = {"form": form, "categories": category,
                   "locations": location, "tusers": tuser}
